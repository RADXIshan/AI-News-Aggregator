...
def get_recipients_from_db() -> List[str]:
    """Fetch active email recipients from the database"""
    from app.database.repository import Repository
    
    repo = Repository()
    email_records = repo.get_all_emails(active_only=True)
    return [record.email for record in email_records]


def send_email(subject: str, body_text: str, body_html: str = None, recipients: list = None, use_db_recipients: bool = False):
    """
    Send an email to specified recipients or fetch from database
    ...
    Args:
        ...
        use_db_recipients: If True, fetch recipients from database (default: False)
    """
    if use_db_recipients:
        db_recipients = get_recipients_from_db()
        if db_recipients:
            recipients = db_recipients
        else:
            print("Warning: No active recipients found in database, falling back to MY_EMAIL")
            recipients = None
...
def digest_to_html(digest_response) -> str:
    ...
    for article in digest_response.articles:
        ...
        html_parts.append(f'<div class="article-summary">{summary_html}</div>')
